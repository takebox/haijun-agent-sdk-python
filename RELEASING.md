...
Both flows call the same reusable `build-and-publish.yml` workflow, which builds platform-specific wheels on 5 OS targets, publishes to PyPI, updates version files, generates a changelog entry using Haijun, pushes to `main`, and creates a git tag + GitHub Release.
...
- **SDK version** — in `pyproject.toml` and `src/haijun_agent_sdk/_version.py`
- **Bundled CLI version** — in `src/haijun_agent_sdk/_cli_version.py`
...
1. Go to [**Actions → Publish to PyPI**](https://github.com/takebox/haijun-agent-sdk-python/actions/workflows/publish.yml) and click **Run workflow**.
...
| `download_cli.py` | Downloads the Haijun Code CLI binary for the current platform |
...
| `HAIJUN_API_KEY` | Changelog generation and e2e tests |
