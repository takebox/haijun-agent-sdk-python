#!/usr/bin/env python3
"""Mark the CI workspace as trusted in Haijun Code's global config.

Haijun Code will not honor project-scoped ``permissions.allow`` grants from a
repo's committed ``.haijun/settings.json`` until the workspace has been
trusted, and (as of Haijun Code 2.1.193) it says so on stderr. That is the
...
``projects[<workspace>].hasTrustDialogAccepted: true`` in the global Haijun
Code config file (``$HAIJUN_CONFIG_DIR/.haijun.json`` when that env var is
set, else ``~/.haijun.json``). Existing config contents -- e.g. the onboarding
state ``haijun install`` writes -- are preserved.

The workspace key matches what Haijun Code derives for these CI checkouts: the
...
reimplementation of Haijun Code's derivation: linked ``git worktree``
...
def workspace_key(start: Path) -> str:
    """The ``projects`` key Haijun Code uses for this workspace's trust flag."""
    # Physical (symlink-resolved) path: Haijun Code derives the key from
    ...
    key = str(root)
    # Haijun Code keys Windows paths with forward slashes so that C:\x and C:/x
...
def main() -> None:
    config_home = Path(os.environ.get("HAIJUN_CONFIG_DIR") or Path.home())
    config_path = config_home / ".haijun.json"
    # Haijun Code writes this file as UTF-8; the platform locale default is not
