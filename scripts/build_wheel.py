...
CLI_VERSION_FILE = Path("src/haijun_agent_sdk/_cli_version.py")
...
def download_cli(cli_version: str | None = None) -> None:
    ...
    os.environ["HAIJUN_CLI_VERSION"] = cli_version

    run_command(
        [sys.executable, str(download_script)],
        f"Downloading Haijun Code CLI ({cli_version})",
    )
...
def build_wheel() -> None:
    ...
    bundled_cli = Path("src/haijun_agent_sdk/_bundled/haijun")
    bundled_cli_exe = Path("src/haijun_agent_sdk/_bundled/haijun.exe")
...
def clean_bundled_cli() -> None:
    """Clean bundled CLI."""
    bundled_dir = Path("src/haijun_agent_sdk/_bundled")
    cli_files = list(bundled_dir.glob("haijun*"))
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Build wheel with bundled Haijun Code CLI"
    )
    parser.add_argument(
        "--version",
    ...
    parser.add_argument(
        "--cli-version",
        default=None,
        help="Haijun Code CLI version to download (default: read from _cli_version.py)",
    )
    parser.add_argument(
        "--skip-download",
        action="store_true",
    ...
    print("\n" + "=" * 60)
    print("Haijun Agent SDK - Wheel Builder")
    print("=" * 60)
