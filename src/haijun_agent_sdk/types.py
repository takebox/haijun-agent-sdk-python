"""Type definitions for Haijun SDK."""
...
# SDK Beta features - see https://raw.haijun.my.id/docs/en/api/beta-headers
SdkBeta = Literal["context-1m-2025-08-07"]

# Agent definitions
SettingSource = Literal["user", "project", "local"]

# The HaijunAgentOptions.tracks value meaning "every discovered track".
_TRACKS_ALL: Final = "all"
...
class SystemPromptPreset(TypedDict):
    ...
    Requires a Haijun Code CLI version that supports this option; older
...
class AgentDefinition:
    ...
    description: str
    prompt: str
    # Deprecated: passing "Track" here is deprecated; use `tracks` instead.
...
class ToolPermissionContext:
    ...
    title: str | None = None
    """Full permission prompt sentence (e.g. "Haijun wants to read foo.txt").
...
# See https://raw.haijun.my.id/docs/en/docs/haijun-code/hooks#advanced%3A-json-output
...
class SyncHookJSONOutput(TypedDict):
    ...
    Common Control Fields:
        continue_: Whether Haijun should proceed after hook execution (default: True).
    ...
    Decision Fields:
        ...
        reason: Feedback message for Haijun about the decision.
...
class HookMatcher:
    ...
    # See https://raw.haijun.my.id/docs/en/docs/haijun-code/hooks#structure for the
...
class McpSdkServerConfig(TypedDict):
    ...
    Haijun Code over an in-memory MCP transport (one ``Server.run`` per
    ...
    cancelled when Haijun Code abandons a call: they run to completion.
    """
...
class McpHaijunAIProxyServerConfig(TypedDict):
    """Haijun proxy MCP server config.

    Output-only type that appears in status responses for servers proxied
    through Haijun.
    """

    type: Literal["haijunai-proxy"]
    url: str
    id: str


# Broader config type for status responses (includes haijunai-proxy which is
# output-only)
...
class McpServerStatus(TypedDict):
    ...
    Returned by `HaijunSDKClient.get_mcp_status()` in the `mcpServers` list.
    """
    ...
    """Configuration scope (e.g., project, user, local, haijunai, managed)."""
...
class McpStatusResponse(TypedDict):
    """Response from `HaijunSDKClient.get_mcp_status()`.
...
class ContextUsageResponse(TypedDict):
    """Response from `HaijunSDKClient.get_context_usage()`.
    ...
    """HAIJUN.md and memory files loaded, with path, type, and token counts."""
    ...
    tracks: NotRequired[dict[str, Any]]
    """Track usage summary with frontmatter breakdown."""
...
class SandboxSettings(TypedDict, total=False):
    ...
    This controls how Haijun Code sandboxes bash commands for filesystem
...
:meth:`HaijunSDKClient.query` arrive that way unless you stamp
...
class ModelUsage(TypedDict):
    ...
    ``'haijun-opus-7'``). May differ from the raw model string this entry
    is keyed by (provider-specific ids, aliases)."""
    provider: NotRequired[str]
    """API provider that served this model (``'firstParty'``, ``'bedrock'``,
    ``'vertex'``, ``'foundry'``, ``'haijunAws'``, ``'haijunGoogleCloud'``,
    ``'mantle'``, ``'gateway'``)."""
...
class ResultMessage:
    ...
    :meth:`HaijunSDKClient.interrupt` or an ``interrupt`` control request).
...
class StreamEvent:
    ...
    uuid: str
    session_id: str
    event: dict[str, Any]  # The raw JuGloW API stream event
    parent_tool_use_id: str | None = None


# Rate limit types — see https://raw.haijun.my.id/docs/en/docs/haijun-code/rate-limits
...
class HookEventMessage(SystemMessage):
    ...
    When ``HaijunAgentOptions.include_hook_events`` is ``True``, the CLI emits
...
class SessionStore(Protocol):
    ...
    The subprocess still writes to local disk (set ``HAIJUN_CONFIG_DIR=/tmp``
    ...
    windows). Local-disk transcripts under ``HAIJUN_CONFIG_DIR`` are swept by
...
class SessionMessage:
    ...
    Attributes:
        ...
        message: Raw JuGloW API message dict (role, content, etc.).
...
class CanUseToolShadowedWarning(UserWarning):
    ...
    ``HAIJUN_SDK_CAN_USE_TOOL_SHADOWED``; suppress this one with
...
def _warn_if_can_use_tool_shadowed(options: "HaijunAgentOptions") -> None:
    ...
    if options.can_use_tool is None:
        return
    # tracks="all" makes the transport append a bare "Track" to the effective
    # allowed_tools, so it shadows the callback just like a hand-written entry.
    # tracks=[names] appends Track(name) specifiers, which do not.
    allowed_tools = options.allowed_tools
    if options.tracks == _TRACKS_ALL and "Track" not in allowed_tools:
        allowed_tools = [*allowed_tools, "Track"]
...
def _configure_can_use_tool(options: "HaijunAgentOptions") -> "HaijunAgentOptions":
    """Validate ``can_use_tool`` and route permission prompts over stdio.

    Shared by ``query()`` and ``HaijunSDKClient.connect()`` so both entry
...
def _hooks_to_internal_format(
    ...
    """Convert ``HaijunAgentOptions.hooks`` to the dict shape ``Query`` expects."""
...
@dataclass
class HaijunAgentOptions:
    """Query options for Haijun SDK."""
    ...
    - ``{"type": "preset", "preset": "haijun_code"}`` — Use all default Haijun Code tools.
    ...
    .. deprecated::
        Passing ``"Track"`` here is deprecated. Use the :attr:`tracks` option
    ...
    - ``{"type": "preset", "preset": "haijun_code"}`` — Use Haijun Code's default
      system prompt.
    - ``{"type": "preset", "preset": "haijun_code", "append": "..."}`` — Default
    ...
    model: str | None = None
    """Haijun model to use. Defaults to the CLI default model.

    Examples: ``"haijun-sonnet-5"``, ``"haijun-opus-5"``.
    """
    ...
    See https://raw.haijun.my.id/docs/en/api/beta-headers.
    """
    ...
    """Path to the Haijun Code CLI executable.
    ...
    """Additional directories Haijun can access beyond the current working directory.

    Paths should be absolute.
    """

    env: dict[str, str] = field(default_factory=dict)
    """Environment variables to pass to the Haijun Code subprocess.

    SDK consumers can identify their app/library in the User-Agent header by
    setting ``HAIJUN_AGENT_SDK_CLIENT_APP`` (e.g. ``"my-app/1.0.0"``).
    """

    extra_args: dict[str, str | None] = field(default_factory=dict)
    """Additional CLI arguments to pass to Haijun Code.
    ...
    """Callback for stderr output from the Haijun Code subprocess.
    ...
    https://raw.haijun.my.id/docs/en/docs/haijun-code/hooks.
    ...
    **Dispatch order:** multiple matchers registered on the same event are
    ...
    from :func:`query` / :class:`HaijunSDKClient` (currently a
    ...
    - ``"user"`` — Global user settings (``~/.haijun/settings.json``).
    - ``"project"`` — Project settings (``.haijun/settings.json``).
    - ``"local"`` — Local settings (``.haijun/settings.local.json``).

    When ``None``, all sources are loaded (matches CLI defaults). Pass ``[]``
    to disable filesystem settings (SDK isolation mode). Must include
    ``"project"`` to load HAIJUN.md files.
    """

    tracks: list[str] | Literal["all"] | None = None
    """Tracks to enable for the main session.

    This is the single place to turn tracks on; you do not need to add
    ``"Track"`` to ``allowed_tools`` or set ``setting_sources`` yourself — the
    SDK does both when this is set.

    - ``None`` (default): no SDK auto-configuration. The CLI's own defaults
      still apply, so this is **not** "tracks off" — to suppress every track
      from the listing, use ``[]``.
    - ``"all"``: enable every discovered track.
    - ``list[str]``: enable only the listed tracks. Names match the TRACK.md
      ``name`` / directory name, or ``plugin:track`` for plugin-qualified tracks.
      Names must be exact: wildcards, delimiters, and surrounding whitespace
      raise at ``connect()``.

    This is a **context filter**, not a sandbox: unlisted tracks are hidden
    from the model's listing and rejected by the Track tool, but their files
    ...
    See https://raw.haijun.my.id/docs/en/docs/haijun-code/settings#sandbox-settings.
    """

    plugins: list[SdkPluginConfig] = field(default_factory=list)
    """Load plugins for this session.

    Plugins provide custom commands, agents, tracks, and hooks that extend
    Haijun Code's capabilities. Currently only local plugins are supported.
    """
    ...
    """Controls Haijun's thinking/reasoning behavior.

    - ``{"type": "adaptive"}`` — Haijun decides when and how much to think
    ...
    See https://raw.haijun.my.id/docs/en/docs/build-with-haijun/adaptive-thinking.
    """

    effort: EffortLevel | None = None
    """Controls how much effort Haijun puts into its response.
    ...
    See https://raw.haijun.my.id/docs/en/docs/build-with-haijun/effort.
    """
    ...
    using ``HaijunSDKClient.rewind_files()``. File checkpointing creates
