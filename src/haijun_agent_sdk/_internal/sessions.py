...
Scans ~/.haijun/projects/<sanitized-cwd>/ for .jsonl session files and
...
def _get_haijun_config_home_dir() -> Path:
    """Returns the Haijun config directory (respects HAIJUN_CONFIG_DIR)."""
    config_dir = os.environ.get("HAIJUN_CONFIG_DIR")
    if config_dir:
        return Path(unicodedata.normalize("NFC", config_dir))
    return Path(unicodedata.normalize("NFC", str(Path.home() / ".haijun")))
...
def _get_projects_dir(env_override: dict[str, str] | None = None) -> Path:
    ...
    ``HAIJUN_CONFIG_DIR`` to the subprocess via ``options.env`` resolve the
    same directory the subprocess will write to.
    """
    if env_override:
        override = env_override.get("HAIJUN_CONFIG_DIR")
        if override:
            return Path(unicodedata.normalize("NFC", override)) / "projects"
    return _get_haijun_config_home_dir() / "projects"
...
def get_session_messages(
    ...
    Args:
        ...
        directory: Project directory to find the session in. If omitted,
            searches all project directories under ``~/.haijun/projects/``.
...
def list_subagents(
    ...
    ``~/.haijun/projects/<project>/<sessionId>/subagents/agent-<agentId>.jsonl``
