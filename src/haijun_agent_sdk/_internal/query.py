...
def _error_result_text(message: dict[str, Any]) -> str:
    ...
    contradictory "Haijun Code returned an error result: success". Prefer
...
class Query:
    ...
    def __init__(
        ...
        Args:
            ...
            tracks: Optional track allowlist to send via initialize so the CLI
                can filter which tracks are loaded into the system prompt
    ...
    async def _read_messages(self) -> None:
        ...
        try:
            ...
            if isinstance(e, ProcessError) and self._last_error_result is not None:
                error_text = (
                    f"Haijun Code returned an error result: "
    ...
    async def close(self) -> None:
        ...
        - ``transport.close()``. For a custom ``Transport`` (accepted by
          ``query(transport=...)`` and ``HaijunSDKClient(transport=...)``) that
