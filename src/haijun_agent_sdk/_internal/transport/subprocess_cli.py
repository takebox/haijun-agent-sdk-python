"""Subprocess transport implementation using Haijun Code CLI."""
...
MINIMUM_HAIJUN_CODE_VERSION = "2.0.0"
...
# prevents orphaned `haijun` processes from leaking when callers crash or exit
...
# control characters (C0, DEL, C1) never appear in a track directory name.
...
_TRACK_NAME_INVALID_CHARS = re.compile(r"[(),\x00-\x1f\x7f-\x9f\ufeff]")
...
def _reject_non_list_tracks(tracks: object) -> None:
    ...
    track filter at all.
    """
    if isinstance(tracks, list) or tracks == _TRACKS_ALL:
        return
    suggestion = f" Did you mean [{tracks!r}]?" if isinstance(tracks, str) else ""
    raise TypeError(
        "HaijunAgentOptions.tracks must be a list of track names or"
        f' "all", got {tracks!r}.{suggestion}'
    )


def _validate_track_name(name: str) -> None:
    """Reject track names that cannot ride safely in a ``Track(name)`` rule.

    Names from ``options.tracks`` are formatted into the ``--allowedTools``
    ...
    Names that tokenize cleanly but can never match the listed track are
    ...
    if not isinstance(name, str):
        raise TypeError(
            f"Track names must be strings, got {type(name).__name__}: {name!r}"
        )
    if not name.strip():
        raise ValueError("Track names must be non-empty strings")
    if _SURROGATE_RE.search(name):
        raise ValueError(
            f"Invalid track name {name!r}: contains a surrogate code point,"
            " which can never match a track the CLI discovered."
        )
    if name != name.strip():
        raise ValueError(
            f"Invalid track name {name!r}: leading or trailing whitespace"
            " can never match — the Track tool trims the invoked name."
        )
    if _TRACK_NAME_INVALID_CHARS.search(name):
        raise ValueError(
            f"Invalid track name {name!r}: parentheses, commas, control"
            " characters, and byte-order marks are not allowed. Names match"
            " the track's directory name, or 'plugin:track' for"
            " plugin-qualified tracks."
        )
    if name == "*":
        raise ValueError(
            "Invalid track name '*': use tracks=\"all\" to enable every track."
        )
    if name.endswith(":*") or name.endswith(" *"):
        raise ValueError(
            f"Invalid track name {name!r}: wildcard-suffix names are not"
            " allowed; list each track by its exact name."
        )
    if name.startswith("/"):
        raise ValueError(
            f"Invalid track name {name!r}: track names may not start with"
            " '/'. The tracks option takes the canonical name, not the"
            " slash-command form."
        )
    if "\\\\" in name:
        raise ValueError(
            f"Invalid track name {name!r}: consecutive backslashes are not"
            " allowed — the per-rule parser collapses them, so the rule"
            " would name a different track."
        )
    if name.endswith("\\"):
        raise ValueError(
            f"Invalid track name {name!r}: names may not end with an"
            " unpaired backslash."
        )
...
class SubprocessCLITransport(Transport):
    """Subprocess transport using Haijun Code CLI."""
    ...
    def _find_cli(self) -> str:
        ...
        if cli := shutil.which("haijun"):
            ...
            # as the CLI: npm's haijun.cmd shim (which connect() refuses to
            ...
            # directory shadows a native haijun.exe installed in a later
            ...
            # explanatory batch-script refusal from connect(). The haijun.exe
            # probe is vetted too: PATHEXT resolution can append an
            # extension and hand back "haijun.exe.cmd".
            exe = shutil.which("haijun.exe")
            if exe and self._is_windows_native_exe(exe):
                return exe
            which_hit = cli

        if platform.system() == "Windows":
            # Only the native installer's haijun.exe. Path.exists() does
        ...
        if platform.system() == "Windows":
            # npm's Windows install is a haijun.cmd shim, which connect()
            # refuses (_reject_windows_batch_cli), so do not recommend it.
            raise CLINotFoundError(
                "Haijun Code not found. Install the native haijun.exe with "
                "(PowerShell):\n"
                "  irm https://downloads.haijun.my.id/cli/install.ps1 | iex\n"
                "\nOr install the haijun-agent-sdk wheel for a platform that "
                "bundles haijun.exe (e.g. Windows x64), or provide the path to "
                "a haijun.exe via HaijunAgentOptions:\n"
                "  HaijunAgentOptions(cli_path='C:\\\\path\\\\to\\\\haijun.exe')\n"
                "\n(npm install -g @takebox-ai/haijun-code produces a haijun.cmd "
        ...
        raise CLINotFoundError(
            ...
            "\nOr provide the path via HaijunAgentOptions:\n"
            "  HaijunAgentOptions(cli_path='/path/to/haijun')"
        )

    def _find_bundled_cli(self) -> str | None:
        """Find bundled CLI binary if it exists."""
        # Determine the CLI binary name based on platform
        cli_name = "haijun.exe" if platform.system() == "Windows" else "haijun"
        ...
        if bundled_path.exists() and bundled_path.is_file():
            logger.info(f"Using bundled Haijun Code CLI: {bundled_path}")
    ...
    def _is_windows_batch_cli(cli_path: str) -> bool:
        ...
        # "haijun.cmd\\...\\.." resolves to haijun.cmd on Windows while the
        ...
        # nothing legitimate: no real haijun.exe lives beneath a directory
        ...
        # "haijun:evil.cmd" has extension ".cmd" -- while an NTFS stream
        # spec also opens its base file -- "haijun.cmd:stream" opens
        # haijun.cmd -- and a drive prefix ("C:haijun.cmd") rides in the
    ...
    def _reject_windows_batch_cli(cli_path: str) -> None:
        ...
        In practice this refuses npm's haijun.cmd shim, which _find_cli
        returns only when no native haijun.exe is discoverable (for
        ...
        raise CLIConnectionError(
            ...
            "cmd.exe exists. Use a native haijun executable instead: "
            "install Haijun Code natively "
            "(irm https://downloads.haijun.my.id/cli/install.ps1 | iex), point "
            "HaijunAgentOptions(cli_path=...) at a haijun.exe, or install "
            "the haijun-agent-sdk wheel for a platform that bundles "
            "haijun.exe (e.g. Windows x64)."
        )
    ...
    def _apply_tracks_defaults(
        self,
    ) -> tuple[list[str], list[str] | None]:
        """Compute effective allowed_tools and setting_sources for tracks.

        When ``options.tracks`` is ``"all"``, injects the bare ``Track`` tool;
        when it is a list, injects ``Track(name)`` for each entry. In either
        case ``setting_sources`` defaults to ``["user", "project"]`` when
        unset so the CLI discovers installed tracks without the caller having
        to wire up both options manually. ``None`` is a no-op.

        Each listed track name is validated before being formatted into a
        rule; see :func:`_validate_track_name`.
        ...
        tracks = self._options.tracks
        if tracks is None:
            return allowed_tools, setting_sources
        _reject_non_list_tracks(tracks)

        if tracks == _TRACKS_ALL:
            if "Track" not in allowed_tools:
                allowed_tools.append("Track")
        else:
            for name in tracks:
                _validate_track_name(name)
                pattern = f"Track({name})"
    ...
    def _build_command(self) -> list[str]:
        ...
        if self._options.tools is not None:
            ...
            if isinstance(tools, list):
                ...
                # Preset object - 'haijun_code' preset maps to 'default'
                cmd.extend(["--tools", "default"])

        effective_allowed_tools, effective_setting_sources = (
            self._apply_tracks_defaults()
        )
    ...
    async def connect(self) -> None:
        ...
        if not os.environ.get("HAIJUN_AGENT_SDK_SKIP_VERSION_CHECK"):
            await self._check_haijun_version()

        cmd = self._build_command()
        try:
            # Merge environment variables. HAIJUN_CODE_ENTRYPOINT defaults to
            # sdk-py regardless of inherited process env; options.env can override
            # it. HAIJUN_AGENT_SDK_VERSION is always set by the SDK.
            # Filter out HAIJUNCODE so SDK-spawned subprocesses don't think
            # they're running inside a Haijun Code parent (see #573).
            inherited_env = {k: v for k, v in os.environ.items() if k != "HAIJUNCODE"}
            process_env = {
                **inherited_env,
                "HAIJUN_CODE_ENTRYPOINT": "sdk-py",
                **self._options.env,
                "HAIJUN_AGENT_SDK_VERSION": __version__,
            }
            ...
            try:
                ...
                if "traceparent" in carrier:
                    ...
                    # TRACEPARENT. Explicit HaijunAgentOptions.env always wins.
            ...
            if self._options.enable_file_checkpointing:
                process_env["HAIJUN_CODE_ENABLE_SDK_FILE_CHECKPOINTING"] = "true"
            ...
            if self._cwd and not Path(self._cwd).exists():
                ...
                raise error from e
            error = CLINotFoundError(f"Haijun Code not found at: {self._cli_path}")
            self._exit_error = error
            raise error from e
        except Exception as e:
            error = CLIConnectionError(f"Failed to start Haijun Code: {e}")
            self._exit_error = error
            raise error from e
    ...
    async def close(self) -> None:
        ...
        `async with HaijunSDKClient()` unwinding on cancel), and an
    ...
    async def _check_haijun_version(self) -> None:
        """Check Haijun Code version and warn if below minimum."""
        if self._cli_path is None:
            raise CLINotFoundError("CLI path not resolved. Call connect() first.")
            ...
            with anyio.fail_after(2):  # 2 second timeout
                ...
                if version_process.stdout:
                    ...
                    if match:
                        ...
                        min_parts = [
                            int(x) for x in MINIMUM_HAIJUN_CODE_VERSION.split(".")
                        ]

                        if version_parts < min_parts:
                            logger.warning(
                                "Haijun Code version %s at %s is unsupported in the Agent SDK. "
                                "Minimum required version is %s. "
                                "Some features may not work correctly.",
                                version,
                                self._cli_path,
                                MINIMUM_HAIJUN_CODE_VERSION,
                            )
        except Exception:
            pass
        finally:
