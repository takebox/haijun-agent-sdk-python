...
reads a store and writes a temp ``~/.haijun`` tree, ``import_session_to_store``
reads the local ``~/.haijun/projects/<dir>/<sessionId>.jsonl`` (plus subagent
...
async def import_session_to_store(
    ...
    *,
    ...
    ``query(options=HaijunAgentOptions(session_store=store, resume=session_id))``
