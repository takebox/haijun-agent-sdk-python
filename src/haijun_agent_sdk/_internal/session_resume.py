"""Materialize a :class:`SessionStore`-backed resume into a temp ``HAIJUN_CONFIG_DIR``.
...
``~/.haijun/``, and returns the path so the caller can point the subprocess at
it via ``HAIJUN_CONFIG_DIR``.
...
from ..types import HaijunAgentOptions, SessionKey, SessionStore, SessionStoreFlushMode
...
# HAIJUN_CONFIG_DIR is unset (production OAUTH_FILE_SUFFIX is empty).
_KEYCHAIN_SERVICE_NAME = "Haijun Code-credentials"
...
class MaterializedResume:
    ...
    Attributes:
        config_dir: Temporary directory laid out like ``~/.haijun/`` —
            point the subprocess at it via ``HAIJUN_CONFIG_DIR``.
...
def apply_materialized_options(
    options: HaijunAgentOptions, materialized: MaterializedResume
) -> HaijunAgentOptions:
    """Return a copy of ``options`` repointed at a materialized temp config dir.

    Sets ``HAIJUN_CONFIG_DIR`` in ``env``, ``resume`` to the materialized
    ...
    return replace(
        ...
        env={
            **options.env,
            "HAIJUN_CONFIG_DIR": str(materialized.config_dir),
        },
...
async def materialize_resume_session(
    ...
    tmp_base = Path(tempfile.mkdtemp(prefix="haijun-resume-"))
    try:
        project_dir = tmp_base / "projects" / project_key
        project_dir.mkdir(parents=True, exist_ok=True)
        _write_jsonl(project_dir / f"{session_id}.jsonl", entries)

        # The subprocess will run with HAIJUN_CONFIG_DIR=tmp_base. Copy auth
...
def _copy_auth_files(tmp_base: Path, opt_env: dict[str, str]) -> None:
    ...
    (refreshToken redacted), ``.haijun.json``, and user ``settings.json`` /
    ...
    - ``.credentials.json``, ``settings.json`` and ``cowork_settings.json`` live
      under the config dir (default ``~/.haijun/``)
    - ``.haijun.json`` lives at ``$HAIJUN_CONFIG_DIR/.haijun.json`` when set,
      else ``~/.haijun.json`` (NOT ``~/.haijun/.haijun.json``)
    """
    caller_config_dir = opt_env.get("HAIJUN_CONFIG_DIR") or os.environ.get(
        "HAIJUN_CONFIG_DIR"
    )
    source_config_dir = (
        Path(caller_config_dir) if caller_config_dir else Path.home() / ".haijun"
    )
    ...
    # Redirecting HAIJUN_CONFIG_DIR changes the Keychain service-name suffix,
    ...
    if (
        ...
        and not (opt_env.get("HAIJUN_API_KEY") or os.environ.get("HAIJUN_API_KEY"))
        and not (
            opt_env.get("HAIJUN_CODE_OAUTH_TOKEN")
            or os.environ.get("HAIJUN_CODE_OAUTH_TOKEN")
        )
    ):
    ...
    haijun_json_src = (
        Path(caller_config_dir) / ".haijun.json"
        if caller_config_dir
        else Path.home() / ".haijun.json"
    )
    _copy_if_present(haijun_json_src, tmp_base / ".haijun.json")
...
# User-settings keys that only misbehave under the redirected HAIJUN_CONFIG_DIR:
...
def _strip_settings_for_resume(content: bytes) -> bytes:
    ...
    Removes :data:`_RESUME_SETTINGS_STRIPPED_KEYS` and ``env.HAIJUN_CONFIG_DIR``
    ...
    if isinstance(env_block, dict) and "HAIJUN_CONFIG_DIR" in env_block:
        del env_block["HAIJUN_CONFIG_DIR"]
        stripped = True
    if not stripped:
        return content
    try:
...
def _write_redacted_credentials(creds_json: str | None, dst: Path) -> None:
    """Write ``creds_json`` with ``haijunAiOauth.refreshToken`` removed.

    The resumed subprocess runs under a redirected ``HAIJUN_CONFIG_DIR``; if it
    ...
    try:
        ...
        oauth = data.get("haijunAiOauth") if isinstance(data, dict) else None
