...
from .client import HaijunSDKClient
...
class ToolAnnotations(_McpToolAnnotations, extra="allow"):
    ...
    a Haijun Code one: the size, in characters, up to which Haijun Code keeps
...
def tool(
    ...
    Args:
        name: Unique identifier for the tool. This is what Haijun will use
            to reference the tool in function calls.
        description: Human-readable description of what the tool does.
            This helps Haijun understand when to use the tool.
        ...
        annotations: Optional MCP tool annotations (hints such as
            ``readOnlyHint`` or ``destructiveHint``) advertised to Haijun.
            ``ToolAnnotations(maxResultSizeChars=N)`` additionally raises the
            size up to which Haijun Code keeps this tool's result inline
    ...
    Notes:
        ...
        - Arguments are validated against the input schema before the function
          is called; invalid input is reported back to Haijun as an error result
...
def _build_meta(tool_def: SdkMcpTool[Any]) -> dict[str, Any] | None:
    ...
    # the size up to which Haijun Code keeps a tool result inline rather than
    ...
    if max_size is None:
        return None
    return {"haijun/maxResultSizeChars": max_size}
...
def create_sdk_mcp_server(
    ...
    Args:
        ...
        tools: List of SdkMcpTool instances created with the @tool decorator.
            These are the functions that Haijun can call through this server.
    ...
    Returns:
        ...
        HaijunAgentOptions.mcp_servers. Its ``instance`` is a regular
        ``mcp.server.Server`` that the SDK connects to Haijun Code over an
    ...
    Notes:
        ...
        - Tool arguments are validated against the tool's input schema, and
          ...
          are all reported to Haijun as error results rather than failing the
          request
        - Works with both mcp 1.x and mcp 2.x

    See Also:
        - tool(): Decorator for creating tool functions
        - HaijunAgentOptions: Configuration for using servers with query()
    """
    tools = tools or []
