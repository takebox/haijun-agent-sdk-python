"""End-to-end tests for structured output with real Haijun API calls.

These tests verify that the output_schema feature works correctly by making
actual API calls to Haijun with JSON Schema validation.
"""
...
async def test_simple_structured_output():
    ...
    async for message in query(
        prompt="Count how many Python files are in src/haijun_agent_sdk/ and check if there are any test files. Use tools to explore the filesystem.",
        options=options,
    ):
