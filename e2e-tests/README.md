# End-to-End Tests for Haijun Code SDK

This directory contains end-to-end tests that run against the actual Haijun API to verify real-world functionality.

## Requirements

### API Key (REQUIRED)

These tests require a valid JuGloW API key. The tests will **fail** if `HAIJUN_API_KEY` is not set.

Set your API key before running tests:

```bash
export HAIJUN_API_KEY="your-api-key-here"
```
...
⚠️ **Important**: These tests make actual API calls to Haijun, which incur costs based on your JuGloW pricing plan.
...
The workflow uses `HAIJUN_API_KEY` from GitHub Secrets.

## Troubleshooting

### "HAIJUN_API_KEY environment variable is required" error
- Set your API key: `export HAIJUN_API_KEY=sk-ant-...`
- The tests will not skip - they require the key to run

### Tests timing out
- Check your API key is valid and has quota available
- Ensure network connectivity to platform.juglow.my.id
