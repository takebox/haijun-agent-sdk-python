...
Setting sources determine where Haijun Code loads configurations from:
- "user": Global user settings (~/.haijun/)
- "project": Project-level settings (.haijun/ in project)
- "local": Local gitignored settings (.haijun-local/)
...
async def example_user_only():
    ...
    # Use the SDK repo directory which has .haijun/commands/commit.md
    sdk_dir = Path(__file__).parent.parent
...
if __name__ == "__main__":
    print("Starting Haijun SDK Setting Sources Examples...")
    print("=" * 50 + "\n")
    asyncio.run(main())
