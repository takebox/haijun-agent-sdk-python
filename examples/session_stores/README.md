...
Reference [`SessionStore`](../../src/haijun_agent_sdk/types.py) implementations
...
`boto3` is **not** a dependency of `haijun-agent-sdk` — install it yourself:

```bash
pip install haijun-agent-sdk boto3
```
...
from haijun_agent_sdk import HaijunAgentOptions, ResultMessage, query
...
Local-disk transcripts under `HAIJUN_CONFIG_DIR` are swept independently by
...
[`examples/session-stores/s3/`](https://github.com/takebox/haijun-agent-sdk-typescript/tree/main/examples/session-stores/s3).
...
```bash
pip install haijun-agent-sdk redis
```
...
Local-disk transcripts under `HAIJUN_CONFIG_DIR` are swept independently by the
...
```bash
pip install haijun-agent-sdk asyncpg
```
...
(which defaults to table `haijun_session_entries`, uses `NULL` rather than
...
`DELETE FROM haijun_session_store WHERE mtime < $cutoff` (or partition the
