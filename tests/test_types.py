"""Tests for Haijun SDK type definitions."""
...
class TestMessageTypes:
    ...
    def test_user_message_creation(self):
        ...
        msg = UserMessage(content="Hello, Haijun!")
        assert msg.content == "Hello, Haijun!"
    ...
    def test_assistant_message_with_text(self):
        ...
        msg = AssistantMessage(content=[text_block], model="haijun-opus-1-20250805")
    ...
    def test_assistant_message_with_thinking(self):
        ...
        msg = AssistantMessage(content=[thinking_block], model="haijun-opus-1-20250805")
        assert len(msg.content) == 1
...
class TestOptions:
    ...
    def test_haijun_code_options_with_tools(self):
    ...
    def test_haijun_code_options_with_permission_mode(self):
        """Test Options with permission mode."""
        options = HaijunAgentOptions(permission_mode="bypassPermissions")
        assert options.permission_mode == "bypassPermissions"

        options_plan = HaijunAgentOptions(permission_mode="plan")
        assert options_plan.permission_mode == "plan"

        options_default = HaijunAgentOptions(permission_mode="default")
        assert options_default.permission_mode == "default"

        options_accept = HaijunAgentOptions(permission_mode="acceptEdits")
        assert options_accept.permission_mode == "acceptEdits"

        options_dont_ask = HaijunAgentOptions(permission_mode="dontAsk")
        assert options_dont_ask.permission_mode == "dontAsk"

        options_auto = HaijunAgentOptions(permission_mode="auto")
        assert options_auto.permission_mode == "auto"

    def test_haijun_code_options_with_system_prompt_string(self):
    ...
    def test_haijun_code_options_with_system_prompt_preset(self):
        """Test Options with system prompt preset."""
        options = HaijunAgentOptions(
            system_prompt={"type": "preset", "preset": "haijun_code"},
        )
        assert options.system_prompt == {"type": "preset", "preset": "haijun_code"}

    def test_haijun_code_options_with_system_prompt_preset_and_append(self):
    ...
    def test_haijun_code_options_with_system_prompt_preset_exclude_dynamic_sections(
        self,
    ):
    ...
    def test_haijun_code_options_with_system_prompt_file(self):
    ...
    def test_haijun_code_options_with_session_continuation(self):
        """Test Options with session continuation."""
        options = HaijunAgentOptions(continue_conversation=True, resume="session-123")
        assert options.continue_conversation is True
        assert options.resume == "session-123"

    def test_haijun_code_options_with_model_specification(self):
        """Test Options with model specification."""
        options = HaijunAgentOptions(
            model="haijun-sonnet-5", permission_prompt_tool_name="CustomTool"
        )
        assert options.model == "haijun-sonnet-5"
...
class TestHookInputTypes:
    ...
    def test_pre_tool_use_hook_input_with_agent_id(self):
        ...
        indistinguishable.
        """
        from haijun_agent_sdk.types import PreToolUseHookInput
    ...
    def test_post_tool_use_hook_input_with_agent_id(self):
        ...
        from haijun_agent_sdk.types import PostToolUseHookInput
...
class TestHookSpecificOutputTypes:
    ...
    def test_pre_tool_use_output_has_additional_context(self):
        ...
        output: PreToolUseHookSpecificOutput = {
            ...
            "additionalContext": "context for haijun",
        }
        assert output["additionalContext"] == "context for haijun"
...
class TestMcpServerStatusTypes:
    ...
    def test_mcp_server_status_connected(self):
        ...
        from haijun_agent_sdk import McpServerStatus
    ...
    def test_mcp_server_status_config_haijunai_proxy(self):
        """Test McpServerStatusConfig accepts haijunai-proxy variant."""
        from haijun_agent_sdk import McpServerStatus
        ...
        assert status["config"]["type"] == "haijunai-proxy"
    ...
    def test_mcp_status_response_wraps_servers(self):
        ...
        from haijun_agent_sdk import McpStatusResponse
...
class TestAgentDefinition:
    ...
    def test_minimal_definition_omits_unset_fields(self):
        from haijun_agent_sdk import AgentDefinition
    ...
    def test_tracks_and_memory_serialize_with_cli_keys(self):
        from haijun_agent_sdk import AgentDefinition
        ...
        assert payload["tracks"] == ["track-a", "track-b"]
    ...
    def test_model_accepts_full_model_id(self):
        ...
        assert payload["model"] == "haijun-opus-5"
