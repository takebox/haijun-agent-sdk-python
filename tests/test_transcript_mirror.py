...
from haijun_agent_sdk._internal.session_resume import build_mirror_batcher
from haijun_agent_sdk._internal.session_store import file_path_to_session_key
from haijun_agent_sdk._internal.sessions import _get_projects_dir
...
PROJECTS_DIR = str(Path(os.sep, "home", "user", ".haijun", "projects"))
...
class TestGetProjectsDirEnvOverride:
    ...
    def test_env_override_takes_precedence(self, tmp_path: Path) -> None:
        ...
        with patch.dict(os.environ, {"HAIJUN_CONFIG_DIR": str(tmp_path / "ambient")}):
            assert _get_projects_dir({"HAIJUN_CONFIG_DIR": str(custom)}) == (
                custom / "projects"
            )
    ...
    def test_falls_back_to_os_environ_when_override_absent(
        ...
        with patch.dict(os.environ, {"HAIJUN_CONFIG_DIR": str(ambient)}):
    ...
    def test_empty_string_override_ignored(self, tmp_path: Path) -> None:
        ambient = tmp_path / "ambient"
        with patch.dict(os.environ, {"HAIJUN_CONFIG_DIR": str(ambient)}):
            assert _get_projects_dir({"HAIJUN_CONFIG_DIR": ""}) == ambient / "projects"
...
# of the ``with`` block.
_BATCHER_SLEEP = "haijun_agent_sdk._internal.transcript_mirror_batcher.anyio.sleep"
...
class TestBuildMirrorBatcherFlushMode:
    ...
    def test_flush_mode_sets_thresholds(
        ...
        batcher = build_mirror_batcher(
            ...
            materialized=None,
            env={"HAIJUN_CONFIG_DIR": str(Path(PROJECTS_DIR).parent)},
            on_error=_noop_error,
            **kwargs,
        )
    ...
    def test_options_default_is_batched(self) -> None:
        assert HaijunAgentOptions().session_store_flush == "batched"
    ...
    def test_flag_present_when_session_store_set(self) -> None:
        transport = SubprocessCLITransport(
            ...
            options=HaijunAgentOptions(
                cli_path="/usr/bin/haijun", session_store=InMemorySessionStore()
            ),
        )
...
_ASSISTANT_MSG = {
    ...
    "message": {
        ...
        "model": "haijun-3-5-sonnet-20241022",
    },
}
    ...
    def test_transcript_mirror_frames_not_yielded_and_store_appended(self) -> None:
        async def _test() -> None:
            ...
            with (
                ...
                patch(
                    "haijun_agent_sdk._internal.session_resume._get_projects_dir",
    ...
    def test_mirror_frames_dropped_when_no_session_store(self) -> None:
        ...
        async def _test() -> None:
            ...
            with (
                ...
                mock_cls.return_value = mock_transport
                messages = [
                    m async for m in query(prompt="Hello", options=HaijunAgentOptions())
                ]
        ...
        async def _test() -> None:
            ...
            with (
                ...
                messages = [
                    ...
                    async for m in query(
                        prompt="Hello",
                        options=HaijunAgentOptions(session_store=FailingStore()),
                    )
                ]
