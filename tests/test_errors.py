"""Tests for Haijun SDK error handling."""
...
class TestErrorTypes:
    ...
    def test_base_error(self):
        """Test base HaijunSDKError."""
        error = HaijunSDKError("Something went wrong")
        assert str(error) == "Something went wrong"
        assert isinstance(error, Exception)

    def test_cli_not_found_error(self):
        """Test CLINotFoundError."""
        error = CLINotFoundError("Haijun Code not found")
        assert isinstance(error, HaijunSDKError)
        assert "Haijun Code not found" in str(error)
    ...
    def test_result_error_carries_payload(self):
        ...
        error = ResultError("Haijun Code returned an error result: x", data, 1)
    ...
    def test_result_error_survives_pickle_and_copy(self):
        ...
        error = ResultError("Haijun Code returned an error result: too many", data, 1)
