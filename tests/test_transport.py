"""Tests for Haijun SDK transport layer."""
...
from haijun_agent_sdk.types import HaijunAgentOptions

DEFAULT_CLI_PATH = "/usr/bin/haijun"
_ABSENT = object()  # sentinel for "field not sent on the wire"


def make_options(**kwargs: object) -> HaijunAgentOptions:
    """Construct options using the standard CLI path unless overridden."""

    cli_path = kwargs.pop("cli_path", DEFAULT_CLI_PATH)
    return HaijunAgentOptions(cli_path=cli_path, **kwargs)
...
class TestSubprocessCLITransport:
    ...
    def test_find_cli_not_found(self):
        ...
        async def _test():
            from haijun_agent_sdk._errors import CLINotFoundError

            transport = SubprocessCLITransport(
                prompt="test", options=HaijunAgentOptions()
            )
            assert transport._cli_path is None

            with (
                patch(
                    "haijun_agent_sdk._internal.transport.subprocess_cli.shutil.which",
                    return_value=None,
                ),
            ...
            assert "Haijun Code not found" in str(exc_info.value)

        anyio.run(_test)

    def test_init_does_not_call_find_cli(self):
        """Test that __init__ defers CLI discovery instead of blocking."""
        transport = SubprocessCLITransport(prompt="test", options=HaijunAgentOptions())
        assert transport._cli_path is None
    ...
    def test_init_uses_provided_cli_path(self):
        ...
        transport = SubprocessCLITransport(
            prompt="test",
            options=HaijunAgentOptions(cli_path="/usr/bin/haijun"),
        )
        assert transport._cli_path == "/usr/bin/haijun"
    ...
    def test_cli_path_accepts_pathlib_path(self):
        ...
        path = Path("/usr/bin/haijun")
        transport = SubprocessCLITransport(
            prompt="Hello",
            options=HaijunAgentOptions(cli_path=path),
        )
        ...
        async def _test():
            with patch("anyio.open_process") as mock_exec:
                ...
                mock_version_process.stdout.receive = AsyncMock(
                    return_value=b"2.0.0 (Haijun Code)"
                )
    ...
    def test_connect_with_nonexistent_cwd(self):
        ...
        from haijun_agent_sdk._errors import CLIConnectionError
    ...
    def test_build_command_tracks_none_leaves_options_untouched(self):
        """When tracks is None (default), neither allowed_tools nor setting_sources change."""
        transport = SubprocessCLITransport(
            prompt="test",
            options=make_options(),
        )
    ...
    def test_build_command_tracks_all_enables_track_tool(self):
        """tracks='all' enables the bare Track tool and defaults setting_sources."""
        transport = SubprocessCLITransport(
            prompt="test",
            options=make_options(tracks="all"),
        )
        cmd = transport._build_command()
        assert "--allowedTools" in cmd
        assert cmd[cmd.index("--allowedTools") + 1] == "Track"
        assert "--setting-sources=user,project" in cmd

    def test_build_command_tracks_empty_list_adds_no_track_entries(self):
        """tracks=[] is a degenerate subset: setting_sources defaults, no Track entries."""
        transport = SubprocessCLITransport(
            prompt="test",
            options=make_options(tracks=[]),
        )
        cmd = transport._build_command()
        assert "--allowedTools" not in cmd
        assert "--setting-sources=user,project" in cmd

    def test_build_command_tracks_named_list_uses_track_patterns(self):
        """Non-empty tracks list adds Track(name) entries and defaults setting_sources."""
        transport = SubprocessCLITransport(
            prompt="test",
            options=make_options(tracks=["pdf", "docx"]),
        )
        cmd = transport._build_command()
        assert "--allowedTools" in cmd
        assert cmd[cmd.index("--allowedTools") + 1] == "Track(pdf),Track(docx)"
        assert "--setting-sources=user,project" in cmd

    def test_build_command_tracks_merges_with_existing_allowed_tools(self):
        """tracks augment (not replace) an existing allowed_tools list."""
        transport = SubprocessCLITransport(
            prompt="test",
            options=make_options(
                allowed_tools=["Read", "Write"],
        ...
        cmd = transport._build_command()
        assert cmd[cmd.index("--allowedTools") + 1] == "Read,Write,Track(pdf)"

    def test_build_command_tracks_preserves_user_setting_sources(self):
        """When setting_sources is explicitly provided, tracks should not override it."""
        transport = SubprocessCLITransport(
            prompt="test",
            options=make_options(
                tracks="all",
    ...
    def test_build_command_tracks_does_not_mutate_options(self):
        """Applying tracks defaults must not mutate the caller's options object."""
        options = make_options(allowed_tools=["Read"], tracks=["pdf"])
        transport = SubprocessCLITransport(prompt="test", options=options)
        transport._build_command()
        assert options.allowed_tools == ["Read"]
        assert options.setting_sources is None

    def test_build_command_tracks_does_not_duplicate_entries(self):
        """Injecting Track entries is idempotent when caller already listed them."""
        transport = SubprocessCLITransport(
            prompt="test",
            options=make_options(
                allowed_tools=["Track(pdf)"],
                tracks=["pdf"],
            ),
        )
        cmd = transport._build_command()
        assert cmd[cmd.index("--allowedTools") + 1] == "Track(pdf)"

    @pytest.mark.parametrize(
        "hostile_name",
        [
            # Names containing rule-syntax delimiters cannot be represented
            # as a single Track(name) entry and must be rejected, never
            # formatted into the --allowedTools value.
            "x),Bash(*",
            "safe),Bash,Track(dummy",
            "name,with,commas",
            "unbalanced(",
            "unbalanced)",
            "()",
    ...
    def test_build_command_tracks_rejects_rule_syntax_delimiters(
        self, hostile_name: str
    ):
        """Track names containing rule-syntax delimiters raise ValueError."""
        transport = SubprocessCLITransport(
            prompt="test",
            options=make_options(tracks=[hostile_name]),
        )
    ...
    def test_build_command_tracks_rejects_control_characters(self, hostile_name: str):
        """Track names containing control characters raise ValueError."""
        transport = SubprocessCLITransport(
            prompt="test",
            options=make_options(tracks=[hostile_name]),
        )
    ...
    def test_build_command_tracks_rejects_empty_names(self, empty_name: str):
        """Empty or whitespace-only track names raise ValueError."""
        transport = SubprocessCLITransport(
            prompt="test",
            options=make_options(tracks=[empty_name]),
        )
        with pytest.raises(ValueError, match="non-empty"):
            transport._build_command()

    def test_build_command_tracks_rejects_non_string_names(self):
        """Non-string entries in the tracks list raise TypeError."""
        transport = SubprocessCLITransport(
            prompt="test",
            options=make_options(tracks=[42]),  # type: ignore[list-item]
        )
    ...
    def test_build_command_tracks_rejects_wildcard_suffix_names(
        self, wildcard_name: str
    ):
        """Wildcard-suffix track names raise ValueError."""
        transport = SubprocessCLITransport(
            prompt="test",
            options=make_options(tracks=[wildcard_name]),
        )
        with pytest.raises(ValueError, match="wildcard-suffix"):
            transport._build_command()

    @pytest.mark.parametrize("tracks", ["pdf", "pdf-tools", "ALL"])
    def test_build_command_tracks_rejects_a_bare_string(self, tracks: str):
        """A string is iterable, so tracks="pdf" would build Track(p),Track(d),..."""
        transport = SubprocessCLITransport(
            prompt="test",
            options=make_options(tracks=tracks),  # type: ignore[arg-type]
        )
        with pytest.raises(TypeError, match="must be a list of track names"):
            transport._build_command()
    ...
    def test_build_command_tracks_rejects_non_list_iterables(self, tracks):
        """These build Track(name) rules but are dropped from initialize, so the
        session track filter is never installed."""
        transport = SubprocessCLITransport(
            prompt="test",
            options=make_options(tracks=tracks),  # type: ignore[arg-type]
        )
    ...
    def test_build_command_tracks_rejects_bare_wildcard(self):
        """A literal '*' name raises, pointing to the tracks="all" option."""
        transport = SubprocessCLITransport(
            prompt="test",
            options=make_options(tracks=["*"]),
        )
        with pytest.raises(ValueError, match='use tracks="all"'):
            transport._build_command()

    def test_build_command_tracks_rejects_unpaired_trailing_backslash(self):
        """A name ending in a single trailing backslash raises ValueError."""
        transport = SubprocessCLITransport(
            prompt="test",
            options=make_options(tracks=["name\\"]),
        )
        with pytest.raises(ValueError, match="unpaired backslash"):
            transport._build_command()

    @pytest.mark.parametrize("hostile_name", ["name\\\\", "name\\\\\\", "mid\\\\dle"])
    def test_build_command_tracks_rejects_consecutive_backslashes(
        self, hostile_name: str
    ):
        """Consecutive backslashes collapse at parse time, renaming the track."""
        transport = SubprocessCLITransport(
            prompt="test",
            options=make_options(tracks=[hostile_name]),
        )
    ...
    def test_build_command_tracks_rejects_byte_order_marks(self, hostile_name: str):
    ...
    def test_build_command_tracks_rejects_surrounding_whitespace(
        self, hostile_name: str
    ):
        """A padded rule can never match: the Track tool trims before matching."""
        transport = SubprocessCLITransport(
            prompt="test",
            options=make_options(tracks=[hostile_name]),
        )
    ...
    def test_build_command_tracks_rejects_leading_slash(self, hostile_name: str):
        """The session allowlist matches verbatim, so '/pdf' hides every track."""
        transport = SubprocessCLITransport(
            prompt="test",
            options=make_options(tracks=[hostile_name]),
        )
    ...
    def test_build_command_tracks_rejects_surrogate_code_points(
        self, hostile_name: str
    ):
        """No CLI-discovered track name contains a surrogate code point."""
        transport = SubprocessCLITransport(
            prompt="test",
            options=make_options(tracks=[hostile_name]),
        )
    ...
    def test_build_command_tracks_rejects_c1_control_characters(
        self, hostile_name: str
    ):
    ...
    def test_build_command_tracks_accepts_ordinary_names(self, benign_name: str):
        """Ordinary names -- plugin-qualified, spaced, non-ASCII -- still work."""
        transport = SubprocessCLITransport(
            prompt="test",
            options=make_options(tracks=[benign_name]),
        )
        cmd = transport._build_command()
        assert cmd[cmd.index("--allowedTools") + 1] == f"Track({benign_name})"

    @pytest.mark.parametrize(
        ("tracks", "extra", "want_tools", "want_sources", "want_init_tracks"),
        [
            # (1) default: no auto-config
            (None, {}, None, None, _ABSENT),
            # (2) old manual way still works (tracks=None, user wires it)
            (
                None,
                {
                    "allowed_tools": ["Track", "Read"],
            ...
            # (3) "all": bare Track, default sources, no wire filter
            ("all", {}, "Track", "user,project", _ABSENT),
            # (4) named subset
            (
                ["pdf", "docx"],
                {},
                "Track(pdf),Track(docx)",
                "user,project",
                ["pdf", "docx"],
            ),
    ...
    def test_tracks_option_matrix(
        self, tracks, extra, want_tools, want_sources, want_init_tracks
    ):
        """Documented behavior table for HaijunAgentOptions.tracks.

        Asserts the full (input) -> (allowedTools, setting_sources,
        initialize.tracks) mapping in one place. See also
        test_query.py::test_initialize_* for the wire-level half.
        """
        transport = SubprocessCLITransport(
            prompt="test",
            options=make_options(tracks=tracks, **extra),
        )
        cmd = transport._build_command()
        ...
        if want_init_tracks is _ABSENT:
            assert not isinstance(tracks, list)
        else:
            assert tracks == want_init_tracks
    ...
    def test_env_vars_passed_to_subprocess(self):
        ...
        async def _test():
            ...
            with patch(
                ...
                assert "HAIJUN_CODE_ENTRYPOINT" in env_passed
                assert env_passed["HAIJUN_CODE_ENTRYPOINT"] == "sdk-py"
    ...
    def test_caller_can_override_entrypoint(self):
        """Test that a caller-supplied HAIJUN_CODE_ENTRYPOINT survives the env merge."""

        async def _test():
            custom_env = {"HAIJUN_CODE_ENTRYPOINT": "custom-caller"}
            options = make_options(env=custom_env)
            ...
            with patch(
                ...
                assert env_passed["HAIJUN_CODE_ENTRYPOINT"] == "custom-caller"

                # HAIJUN_AGENT_SDK_VERSION is still SDK-controlled
                assert "HAIJUN_AGENT_SDK_VERSION" in env_passed
    ...
    def test_otel_trace_context_does_not_override_user_env(self):
        ...
        async def _test():
            ...
            with (
                ...
                # explicit HaijunAgentOptions.env must win over the propagator
    ...
    def test_haijuncode_env_var_not_inherited(self):
        """Test that HAIJUNCODE env var is filtered from the subprocess environment."""

        async def _test():
            options = make_options()

            with (
                patch.dict(os.environ, {"HAIJUNCODE": "1"}),
                patch(
                    "anyio.open_process", new_callable=AsyncMock
                ) as mock_open_process,
            ):
                ...
                # HAIJUNCODE must NOT be inherited from the parent process
                assert "HAIJUNCODE" not in env_passed
    ...
    def test_haijuncode_can_be_set_via_options_env(self):
        """Test that users can explicitly set HAIJUNCODE via options.env."""

        async def _test():
            options = make_options(env={"HAIJUNCODE": "1"})
            ...
            with (
                ...
                assert env_passed.get("HAIJUNCODE") == "1"
    ...
    def test_build_command_with_sandbox_only(self):
        ...
        from haijun_agent_sdk import SandboxSettings
    ...
    def test_build_command_with_tools_preset(self):
        ...
        transport = SubprocessCLITransport(
            prompt="test",
            options=make_options(tools={"type": "preset", "preset": "haijun_code"}),
        )
    ...
    def test_build_command_agents_always_via_initialize(self):
        ...
        """
        from haijun_agent_sdk.types import AgentDefinition
    ...
    def test_version_warning_includes_cli_path(self):
        ...
        async def _test():
            ...
            with (
                ...
                patch(
                    "haijun_agent_sdk._internal.transport.subprocess_cli.logger"
                ) as mock_logger,
                ...
                mock_version_process.stdout = MagicMock()
                mock_version_process.stdout.receive = AsyncMock(
                    return_value=b"1.0.0 (Haijun Code)"
                )
                mock_version_process.terminate = MagicMock()
                mock_version_process.wait = AsyncMock()
                ...
                await transport._check_haijun_version()
    ...
    def test_version_warning_not_emitted_for_current_version(self):
        ...
        async def _test():
            ...
            with (
                ...
                mock_version_process.stdout = MagicMock()
                mock_version_process.stdout.receive = AsyncMock(
                    return_value=b"99.99.99 (Haijun Code)"
                )
                mock_version_process.terminate = MagicMock()
                mock_version_process.wait = AsyncMock()
    ...
    def test_stderr_callback_raise_does_not_terminate_loop(self) -> None:
        ...
        async def _test() -> None:
            ...
            transport = SubprocessCLITransport(
                prompt="x", options=HaijunAgentOptions(stderr=stderr_cb)
            )
    ...
    def test_stderr_line_split_across_chunks_is_reassembled(self) -> None:
        ...
        async def _test() -> None:
            ...
            transport = SubprocessCLITransport(
                prompt="x", options=HaijunAgentOptions(stderr=received.append)
            )
    ...
    def test_stderr_line_without_newline_is_flushed_at_buffer_limit(self) -> None:
        ...
        async def _test() -> None:
            ...
            transport = SubprocessCLITransport(
                prompt="x",
                options=HaijunAgentOptions(stderr=received.append, max_buffer_size=10),
            )
...
class TestAtexitChildCleanup:
    ...
    def test_kill_active_children_terminates_process(self) -> None:
        ...
        from haijun_agent_sdk._internal.transport import subprocess_cli
...
class TestWindowsBatchScriptRefusal:
    ...
    _PLATFORM = "haijun_agent_sdk._internal.transport.subprocess_cli.platform.system"

    def test_npm_cmd_shim_from_which_is_refused(self):
        # Shim-only machine: which("haijun") resolves npm's haijun.cmd and
        # no native haijun.exe is discoverable (which("haijun.exe") -> None,
        ...
        async def _test():
            ...
            def _which(name: str) -> str | None:
                return shim if name == "haijun" else None
    ...
    def test_native_exe_is_preferred_over_shadowing_npm_shim(self):
        ...
        # installer's %USERPROFILE%\.local\bin on PATH, so which("haijun")
        # resolves the haijun.cmd shim -- shutil.which walks PATH
        ...
        # the shadowed native haijun.exe via which("haijun.exe") so connect()
        # proceeds instead of refusing.
        async def _test():
            shim = "C:\\Users\\u\\AppData\\Roaming\\npm\\haijun.CMD"
            native = "C:\\Users\\u\\.local\\bin\\haijun.exe"

            def _which(name: str) -> str | None:
                return {"haijun": shim, "haijun.exe": native}.get(name)
    ...
    def test_haijun_exe_probe_result_is_vetted(self):
        # Python 3.12+ shutil.which appends PATHEXT extensions even to a
        # name that already carries one, so which("haijun.exe") can hand
        # back a stray "haijun.exe.cmd". Discovery must not accept that as
        ...
        async def _test():
            ...
            shim = "C:\\Users\\u\\AppData\\Roaming\\npm\\haijun.CMD"
            junk = "C:\\tools\\haijun.exe.cmd"

            def _which(name: str) -> str | None:
                return {"haijun": shim, "haijun.exe": junk}.get(name)
            ...
            with (
                ...
                patch("pathlib.Path.exists", return_value=False),
                patch("anyio.open_process", new_callable=AsyncMock) as mock_open,
                pytest.raises(CLIConnectionError, match=r"npm\\\\haijun\.CMD"),
            ):
                await transport.connect()
    ...
    def test_extensionless_which_hit_still_prefers_native_exe(self):
        ...
        # extensionless git-bash / WSL wrapper script named "haijun" in an
        # early PATH directory shadows a native haijun.exe installed in a
        ...
        async def _test():
            wrapper = "C:\\Users\\u\\bin\\haijun"
            native = "C:\\Users\\u\\.local\\bin\\haijun.exe"

            def _which(name: str) -> str | None:
                return {"haijun": wrapper, "haijun.exe": native}.get(name)
    ...
    def test_explicit_bat_cli_path_is_refused(self):
        async def _test():
            ...
            transport = SubprocessCLITransport(
                prompt="test",
                options=HaijunAgentOptions(cli_path="C:\\tools\\haijun.bat"),
            )
    ...
    @pytest.mark.parametrize(
        ...
        [
            "C:\\tools\\haijun.cmd.",
            "C:\\tools\\haijun.CMD ",
            "C:\\tools\\haijun.cmd:stream",
            "C:\\tools\\.cmd",
            "C:haijun.cmd",
            "C:/tools/haijun.cmd",
            "\\\\server\\share\\haijun.cmd",
            "C:\\tools\\haijun.cmd\\.",
            "C:\\tools\\haijun.cmd\\x\\..",
            "C:\\tools\\haijun.cmd\\x\\.. ",
            "C:\\tools\\haijun.cmd\\x\\.. .",
            "C:\\tools\\haijun.cmd\\\\.",
            "C:/tools/haijun.cmd//x/..",
            "C:\\tools\\haijun.cmd\\",
            "C:\\tools\\haijun.cmd\\...",
            "C:\\tools\\haijun.cmd\\....",
            "C:\\tools\\haijun:evil.cmd",
            "C:\\tools\\haijun.exe:evil.cmd",
            ":haijun.cmd",
            # A middle dots/spaces-only component is a literal name on Win32
            # (trailing-dot trimming applies to the final segment only), so
            # a following ".." pops that literal and lands on haijun.cmd.
            "C:\\tools\\haijun.cmd\\...\\..",
            "C:\\tools\\haijun.cmd\\. .\\..",
            "C:\\tools\\haijun.cmd\\ \\..",
            "C:\\tools\\haijun.cmd\\.. \\..",
    ...
    def test_suffix_tricks_are_refused(self, cli_path: str):
        async def _test():
            ...
            transport = SubprocessCLITransport(
                prompt="test", options=HaijunAgentOptions(cli_path=cli_path)
            )
        ...
        async def _test():
            transport = SubprocessCLITransport(
                ...
                options=HaijunAgentOptions(
                    cli_path="C:\\Users\\u\\.local\\bin\\haijun.EXE"
                ),
            )
            version_process, main_process = _mock_connect_processes()
    ...
    def test_posix_platforms_are_unchanged(self, system: str):
        async def _test():
            ...
            with (
                ...
                patch(
                    "haijun_agent_sdk._internal.transport.subprocess_cli.shutil.which",
                    return_value="/usr/local/bin/haijun",
                ) as mock_which,
                patch("anyio.open_process", new_callable=AsyncMock) as mock_open,
            ):
                mock_open.side_effect = [version_process, main_process]
            ...
            # POSIX discovery uses the which("haijun") result directly: the
            # native-exe preference is a Windows-only branch, so there is no
            # haijun.exe probe here.
            assert mock_which.call_count == 1
            assert mock_which.call_args.args == ("haijun",)
            assert mock_open.call_count == 2
            assert mock_open.call_args_list[1].args[0][0] == "/usr/local/bin/haijun"

        anyio.run(_test)

    def test_guard_is_a_no_op_off_windows(self):
        with patch(self._PLATFORM, return_value="Linux"):
            SubprocessCLITransport._reject_windows_batch_cli("/odd/haijun.cmd")
    ...
    def test_not_found_message_on_windows_recommends_native_exe(self):
        # The npm route yields a haijun.cmd shim that connect() refuses, so
        # the Windows message must lead with the native haijun.exe install.
        message = self._not_found_message("Windows")
        assert "install.ps1" in message
        assert "haijun.exe" in message
    ...
    def test_not_found_message_off_windows_is_unchanged(self):
        ...
        assert message.startswith(
            "Haijun Code not found. Install with:\n"
    ...
    def test_windows_fallback_skips_posix_shaped_probes(self):
        ...
        # ~/.local/bin/haijun artifact (WSL / git-bash script) and a
        # C:\usr\local\bin\haijun planted on the current drive: the Windows
        # fallback must probe only the native ~/.local/bin/haijun.exe, so
