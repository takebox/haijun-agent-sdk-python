...
Root cause (confirmed via haijun-cli-internal, 2026-03-27):
...
from haijun_agent_sdk._internal.message_parser import parse_message
from haijun_agent_sdk._internal.transport.subprocess_cli import SubprocessCLITransport
from haijun_agent_sdk.types import HaijunAgentOptions, ToolResultBlock, UserMessage

DEFAULT_CLI_PATH = "/usr/bin/haijun"

# Layer-2 threshold as confirmed in haijun-cli-internal toolLimits.ts
...
def _capture_env(transport: SubprocessCLITransport) -> dict[str, str]:
    ...
    async def _run():
        ...
        with (
            patch(
                "haijun_agent_sdk._internal.transport.subprocess_cli.anyio.open_process",
                new_callable=AsyncMock,
                return_value=mock_process,
            ) as mock_open,
            patch(
                "haijun_agent_sdk._internal.transport.subprocess_cli.SubprocessCLITransport._check_haijun_version",
                new_callable=AsyncMock,
            ),
        ):
...
class TestEnvInheritanceAndPrecedence:
    ...
    def test_haijuncode_stripped(self):
        """HAIJUNCODE is stripped so spawned subprocesses don't detect a parent CC."""
        with patch.dict(os.environ, {"HAIJUNCODE": "1", "OTHER_VAR": "kept"}):
            transport = make_transport(env={})
            env = _capture_env(transport)
        assert "HAIJUNCODE" not in env
        assert env.get("OTHER_VAR") == "kept"

    def test_sdk_managed_vars_always_set(self):
        transport = make_transport(env={})
        env = _capture_env(transport)
        assert env.get("HAIJUN_CODE_ENTRYPOINT") == "sdk-py"
        assert "HAIJUN_AGENT_SDK_VERSION" in env

    def test_options_env_cannot_override_sdk_version(self):
        from haijun_agent_sdk._version import __version__

        transport = make_transport(env={"HAIJUN_AGENT_SDK_VERSION": "0.0.0"})
        env = _capture_env(transport)
        assert env.get("HAIJUN_AGENT_SDK_VERSION") == __version__
...
PERSISTED_CONTENT = (
    "<persisted-output>\n"
    "Output too large (73.0KB). Full output saved to: /tmp/.haijun/tool-results/abc123.txt\n"
    "\nPreview (first 2KB):\n" + "x" * 2000 + "\n...\n</persisted-output>"
)
...
class TestToolResultParsing:
    ...
    def test_persisted_output_is_not_full_content(self):
        """Haijun receives only the 2 KB preview, not the original large content."""
        msg = parse_message(_user_message_with_tool_result(PERSISTED_CONTENT))
        assert isinstance(msg, UserMessage)
        blocks = [b for b in msg.content if isinstance(b, ToolResultBlock)]
        content = str(blocks[0].content)
