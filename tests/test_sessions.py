...
@pytest.fixture
def haijun_config_dir(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Creates a temporary ~/.haijun directory and points HAIJUN_CONFIG_DIR at it."""
    config_dir = tmp_path / ".haijun"
    config_dir.mkdir()
    (config_dir / "projects").mkdir()
    monkeypatch.setenv("HAIJUN_CONFIG_DIR", str(config_dir))
    return config_dir
...
class TestListSessions:
    ...
    def test_empty_projects_dir(self, haijun_config_dir: Path):
    ...
    def test_no_config_dir(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
        ...
        monkeypatch.setenv("HAIJUN_CONFIG_DIR", str(tmp_path / "nonexistent"))
        assert list_sessions() == []

    def test_single_session(self, haijun_config_dir: Path, tmp_path: Path):
        ...
        project_dir = _make_project_dir(
            haijun_config_dir, os.path.realpath(project_path)
        )
    ...
    def test_custom_title_wins_summary(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_summary_wins_first_prompt(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_multiple_sessions_sorted_by_mtime(
        self, haijun_config_dir: Path, tmp_path: Path
    ):
    ...
    def test_limit(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_offset_pagination(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_filters_sidechain_sessions(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_filters_empty_sessions(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_filters_non_uuid_filenames(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_ignores_non_jsonl_files(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_list_all_sessions(self, haijun_config_dir: Path):
        """When no directory is given, lists across all projects."""
        proj1 = _make_project_dir(haijun_config_dir, "/some/path/one")
        proj2 = _make_project_dir(haijun_config_dir, "/some/path/two")
    ...
    def test_list_all_sessions_dedupes(self, haijun_config_dir: Path):
        """Duplicate session IDs across projects keep the newest."""
        proj1 = _make_project_dir(haijun_config_dir, "/path/one")
        proj2 = _make_project_dir(haijun_config_dir, "/path/two")
    ...
    def test_nonexistent_project_dir(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_empty_file_filtered(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_include_worktrees_disabled(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_limit_zero_returns_all(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_cwd_from_head_fallback_to_project_path(
        ...
        Path(project_path).mkdir(parents=True)
        canonical = os.path.realpath(project_path)
        project_dir = _make_project_dir(haijun_config_dir, canonical)
...
class TestGetSessionMessages:
    ...
    def test_invalid_session_id(self, haijun_config_dir: Path):
        """Non-UUID session_id returns empty list."""
        assert get_session_messages("not-a-uuid") == []
        assert get_session_messages("") == []

    def test_nonexistent_session(self, haijun_config_dir: Path):
    ...
    def test_simple_chain(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_filters_meta_messages(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_keeps_compact_summary(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_limit_and_offset(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_corrupt_lines_skipped(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_search_all_projects_when_no_dir(self, haijun_config_dir: Path):
    ...
    def test_cycle_detection(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_empty_transcript_file(self, haijun_config_dir: Path, tmp_path: Path):
...
class TestTagExtraction:
    ...
    def test_tag_extracted_from_tail(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_tag_last_wins(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_tag_empty_string_is_none(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_tag_absent(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_tag_ignores_tool_use_inputs(self, haijun_config_dir: Path, tmp_path: Path):
...
class TestGetSessionInfo:
    ...
    def test_found_with_directory(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_found_without_directory(self, haijun_config_dir: Path):
        """Session found by searching all project directories."""
        project_dir = _make_project_dir(haijun_config_dir, "/some/project")
    ...
    def test_returns_none_for_sidechain(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_directory_not_containing_session(
        ...
        dir_a = _make_project_dir(haijun_config_dir, os.path.realpath(project_a))
        _make_project_dir(haijun_config_dir, os.path.realpath(project_b))
    ...
    def test_includes_tag(self, haijun_config_dir: Path, tmp_path: Path):
...
class TestListSubagents:
    ...
    def test_empty_subagents_dir(self, haijun_config_dir: Path, tmp_path: Path):
        """Subagents directory exists but is empty → empty list."""
        project_path = str(tmp_path / "proj")
        Path(project_path).mkdir(parents=True)
        sid, _ = _make_session_with_subagents(
            haijun_config_dir, project_path, agent_ids=[]
        )

        assert list_subagents(sid, directory=project_path) == []

    def test_happy_path(self, haijun_config_dir: Path, tmp_path: Path):
        """Returns agent IDs from agent-*.jsonl filenames."""
        project_path = str(tmp_path / "proj")
        Path(project_path).mkdir(parents=True)
        sid, _ = _make_session_with_subagents(
            haijun_config_dir, project_path, agent_ids=["abc123", "def456"]
        )

        result = list_subagents(sid, directory=project_path)
        assert sorted(result) == ["abc123", "def456"]

    def test_ignores_non_agent_files(self, haijun_config_dir: Path, tmp_path: Path):
        """Non-matching files (.meta.json, other JSONL) are ignored."""
        project_path = str(tmp_path / "proj")
        Path(project_path).mkdir(parents=True)
        sid, subagents_dir = _make_session_with_subagents(
            haijun_config_dir, project_path, agent_ids=["keep"]
        )
    ...
    def test_recurses_into_subdirectories(
        ...
        Path(project_path).mkdir(parents=True)
        sid, subagents_dir = _make_session_with_subagents(
            haijun_config_dir, project_path, agent_ids=["top"]
        )
    ...
    def test_searches_all_projects_without_directory(self, haijun_config_dir: Path):
...
class TestGetSubagentMessages:
    ...
    def test_empty_agent_id(self, haijun_config_dir: Path):
    ...
    def test_nonexistent_agent(self, haijun_config_dir: Path, tmp_path: Path):
        """Agent file not found returns empty list."""
        project_path = str(tmp_path / "proj")
        Path(project_path).mkdir(parents=True)
        sid, _ = _make_session_with_subagents(
            haijun_config_dir, project_path, agent_ids=["other"]
        )
    ...
    def test_simple_chain(self, haijun_config_dir: Path, tmp_path: Path):
        ...
        Path(project_path).mkdir(parents=True)
        sid, subagents_dir = _make_session_with_subagents(
            haijun_config_dir, project_path
        )
    ...
    def test_parent_ids_none_when_sidecar_missing_or_unusable(
        self, haijun_config_dir: Path, tmp_path: Path, meta
    ):
        project_path = str(tmp_path / "proj")
        Path(project_path).mkdir(parents=True)
        sid, subagents_dir = _make_session_with_subagents(
    ...
    def test_skips_corrupt_lines(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_empty_agent_file(self, haijun_config_dir: Path, tmp_path: Path):
