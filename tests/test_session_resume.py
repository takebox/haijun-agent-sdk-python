...
from haijun_agent_sdk._internal.session_resume import (
    MaterializedResume,
    materialize_resume_session,
)
from haijun_agent_sdk._internal.session_store import project_key_for_directory
...
def isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.setattr(
        "haijun_agent_sdk._internal.session_resume._read_keychain_credentials",
        lambda: None,
    )
    return home
...
class TestHappyPath:
    ...
    async def test_credentials_redacted(
        ...
        # Seed a credentials file with a refreshToken under the fake ~/.haijun/.
        config = isolated_home / ".haijun"
        config.mkdir()
        (config / ".credentials.json").write_text(
            json.dumps(
                {"haijunAiOauth": {"accessToken": "at", "refreshToken": "SECRET"}}
            )
        )
        (isolated_home / ".haijun.json").write_text('{"theme":"dark"}')
        ...
        assert creds["haijunAiOauth"]["accessToken"] == "at"
        assert "refreshToken" not in creds["haijunAiOauth"]

        # .haijun.json copied verbatim from ~ (not ~/.haijun/).
        assert (m.config_dir / ".haijun.json").read_text() == '{"theme":"dark"}'
    ...
    async def test_credentials_from_caller_config_dir_env(
        ...
        # options.env HAIJUN_CONFIG_DIR takes precedence over ~ lookup.
        custom = tmp_path / "custom-config"
        custom.mkdir()
        (custom / ".credentials.json").write_text(
            json.dumps({"haijunAiOauth": {"accessToken": "fromenv"}})
        )
        ...
        opts = HaijunAgentOptions(
            ...
            session_store=store,
            resume=SESSION_ID,
            env={"HAIJUN_CONFIG_DIR": str(custom)},
        )
        m = await materialize_resume_session(opts)
        assert m is not None
        creds = json.loads((m.config_dir / ".credentials.json").read_text())
        assert creds["haijunAiOauth"]["accessToken"] == "fromenv"
        await m.cleanup()
    ...
    async def test_credentials_from_keychain_fallback(
        ...
        monkeypatch.setattr(
            "haijun_agent_sdk._internal.session_resume._read_keychain_credentials",
            lambda: json.dumps(
                {"haijunAiOauth": {"accessToken": "kc", "refreshToken": "SECRET"}}
            ),
        )
        ...
        assert m is not None
        creds = json.loads((m.config_dir / ".credentials.json").read_text())
        assert creds["haijunAiOauth"]["accessToken"] == "kc"
        assert "refreshToken" not in creds["haijunAiOauth"]
        await m.cleanup()
    ...
    async def _materialize(
        ...
        opts = HaijunAgentOptions(cwd=cwd, session_store=store, resume=SESSION_ID, **kw)
        m = await materialize_resume_session(opts)
        assert m is not None
        return m
    ...
    async def test_user_settings_materialized(
        ...
        if os.name != "nt":
            ...
            for name in ("settings.json", "cowork_settings.json", ".haijun.json"):
    ...
    async def test_user_settings_from_caller_config_dir_env(
        ...
        # A ~/.haijun/settings.json must NOT win over HAIJUN_CONFIG_DIR.
        (isolated_home / ".haijun").mkdir()
        (isolated_home / ".haijun" / "settings.json").write_text('{"x":1}')

        if via == "options_env":
            m = await self._materialize(
                cwd, project_key, env={"HAIJUN_CONFIG_DIR": str(custom)}
            )
        else:
            monkeypatch.setenv("HAIJUN_CONFIG_DIR", str(custom))
            m = await self._materialize(cwd, project_key)
    ...
    async def test_absent_user_settings_writes_nothing(
        ...
        assert not (m.config_dir / ".haijun.json").exists()
        await m.cleanup()
    ...
    async def test_user_settings_strips_plugins_and_config_dir_env(
        ...
        plugin cache) and env.HAIJUN_CONFIG_DIR (which would redirect config
        ...
        original = {
            ...
            "env": {"HAIJUN_CONFIG_DIR": "/elsewhere", "KEEP": "1"},
    ...
    async def test_malformed_user_settings_copied_through(
        ...
        (isolated_home / ".haijun.json").write_bytes(b"[1, 2]")
        ...
        assert (m.config_dir / ".haijun.json").read_bytes() == b"[1, 2]"
        await m.cleanup()
    ...
    async def test_unreadable_seed_files_do_not_abort_resume(
        ...
        (isolated_home / ".haijun.json").mkdir()
    ...
    async def test_write_jsonl_round_trip(self, tmp_path: Path) -> None:
        ...
        from haijun_agent_sdk._internal.session_resume import _write_jsonl
...
class TestSubkeyMaterialization:
    ...
    async def test_traversal_guards(
        ...
        opts = HaijunAgentOptions(cwd=cwd, session_store=EvilStore(), resume=SESSION_ID)
        m = await materialize_resume_session(opts)
        assert m is not None
...
class TestTimeoutsAndErrors:
    ...
    async def test_non_json_serializable_entry_surfaces_clear_error(
        ...
        opts = HaijunAgentOptions(cwd=cwd, session_store=BadStore(), resume=SESSION_ID)
...
# ---------------------------------------------------------------------------
# Integration: HaijunSDKClient.connect() wiring
# ---------------------------------------------------------------------------
...
class TestClientIntegration:
    ...
    async def test_connect_passes_config_dir_resume_and_suppresses_continue(
        ...
        opts = HaijunAgentOptions(
            ...
            session_store=store,
            continue_conversation=True,
            cli_path="/usr/bin/haijun",
        )
        client = HaijunSDKClient(options=opts)

        with (
            patch(
                "haijun_agent_sdk._internal.transport.subprocess_cli."
        ...
        # overrides.
        transport_opts: HaijunAgentOptions = captured["options"]
        assert transport_opts.resume == SESSION_ID
        assert transport_opts.continue_conversation is False
        config_dir = transport_opts.env["HAIJUN_CONFIG_DIR"]
        ...
        assert "HAIJUN_CONFIG_DIR" not in opts.env
    ...
    async def test_custom_transport_skips_materialization(
        ...
        opts = HaijunAgentOptions(cwd=cwd, session_store=store, resume=SESSION_ID)
        client = HaijunSDKClient(options=opts, transport=_make_mock_transport())
    ...
    async def test_connect_no_materialization_passthrough(
        ...
        opts = HaijunAgentOptions(
            cwd=cwd, resume=SESSION_ID, cli_path="/usr/bin/haijun"
        )
        client = HaijunSDKClient(options=opts)
        ...
        with (
            ...
            await client.connect()
            assert "HAIJUN_CONFIG_DIR" not in captured["options"].env
...
def track_resume_dirs() -> Any:
    """Spy on ``tempfile.mkdtemp`` to capture haijun-resume-* dirs."""
    real_mkdtemp = tempfile.mkdtemp
    created: list[Path] = []

    def spy(*a: Any, **kw: Any) -> str:
        d = real_mkdtemp(*a, **kw)
        if "haijun-resume-" in d:
...
class TestSpawnFailureCleanup:
    ...
    async def test_client_aenter_failure_removes_temp_dir(
        ...
        with (
            ...
            async with HaijunSDKClient(options=opts):
                pass  # pragma: no cover
    ...
    async def test_client_initialize_failure_closes_subprocess_before_cleanup(
        ...
        the temp HAIJUN_CONFIG_DIR is removed and must not leak."""
        store = InMemorySessionStore()
        await store.append(
            {"project_key": project_key, "session_id": SESSION_ID},
            [{"type": "user", "uuid": "u1"}],
    ...
    async def test_connect_cancelled_before_spawn_removes_temp_dir(
        ...
        leave no haijun-resume-* dir on disk.
    ...
    async def test_query_early_break_closes_transport_before_temp_dir_removed(
        ...
        HAIJUN_CONFIG_DIR — which the subprocess is reading/writing — is
        removed."""
        store = InMemorySessionStore()
        await store.append(
            {"project_key": project_key, "session_id": SESSION_ID},
        ...
        opts = HaijunAgentOptions(cwd=cwd, session_store=store, resume=SESSION_ID)
        from haijun_agent_sdk._internal.client import InternalClient
        ...
        assert config_dir_at_close["existed"], (
            "temp HAIJUN_CONFIG_DIR was already gone when transport.close() ran"
        )
        for d in track_resume_dirs:
            assert not d.exists(), f"leaked temp dir {d}"
