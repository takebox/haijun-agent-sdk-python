...
class TestTracksShadowing:
    """tracks="all" makes the transport inject a bare "Track" allow rule."""

    def test_tracks_all_shadows_the_track_tool(self):
        options = HaijunAgentOptions(can_use_tool=_can_use_tool, tracks="all")
        with pytest.warns(CanUseToolShadowedWarning, match="invoked for: Track"):
            _warn_if_can_use_tool_shadowed(options)

    def test_named_tracks_do_not_shadow(self):
        """tracks=[names] injects Track(name) specifiers, which are not whole-tool."""
        options = HaijunAgentOptions(can_use_tool=_can_use_tool, tracks=["reviewer"])
        with warnings.catch_warnings():
            warnings.simplefilter("error", CanUseToolShadowedWarning)
            _warn_if_can_use_tool_shadowed(options)

    def test_tracks_all_does_not_duplicate_explicit_track_entry(self):
        options = HaijunAgentOptions(
            can_use_tool=_can_use_tool, tracks="all", allowed_tools=["Track"]
        )
        with pytest.warns(CanUseToolShadowedWarning) as record:
            _warn_if_can_use_tool_shadowed(options)
        assert "invoked for: Track." in str(record[0].message)

    def test_tracks_all_with_wildcard_track_entry_reports_track_once(self):
        """tracks="all" appends bare Track even if Track(*) is present; dedup it."""
        options = HaijunAgentOptions(
            can_use_tool=_can_use_tool, tracks="all", allowed_tools=["Track(*)"]
        )
        with pytest.warns(CanUseToolShadowedWarning) as record:
    ...
    def test_tracks_all_leaves_caller_allowed_tools_untouched(self):
        """The injected entry must not mutate the caller's list."""
        allowed = ["Read"]
        options = HaijunAgentOptions(
            can_use_tool=_can_use_tool, tracks="all", allowed_tools=allowed
        )
        with pytest.warns(CanUseToolShadowedWarning, match="Read, Track"):
            _warn_if_can_use_tool_shadowed(options)
        assert allowed == ["Read"]
...
class TestWarnIfCanUseToolShadowed:
    ...
    def test_repeat_shadowed_configs_dedupe_under_default_filter(self):
        ...
        """
        same_a = HaijunAgentOptions(can_use_tool=_can_use_tool, allowed_tools=["Read"])
        same_b = HaijunAgentOptions(can_use_tool=_can_use_tool, allowed_tools=["Read"])
        different = HaijunAgentOptions(
...
class TestConnectEmitsWarning:
    ...
    async def test_connect_warns_once_for_shadowed_config(self):
        ...
        client = HaijunSDKClient(options, transport=_create_mock_transport())
        with warnings.catch_warnings(record=True) as record:
            warnings.simplefilter("always")
            try:
