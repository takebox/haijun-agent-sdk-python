...
from haijun_agent_sdk._errors import CLIConnectionError, ProcessError, ResultError
from haijun_agent_sdk._internal.query import Query
from haijun_agent_sdk.types import HookMatcher
...
def test_initialize_sends_tracks_list():
    """Query.initialize() includes tracks only when it is a list."""
    sent = _capture_initialize_request(tracks=["pdf", "docx"])
    assert sent["tracks"] == ["pdf", "docx"]

    sent_empty = _capture_initialize_request(tracks=[])
    assert sent_empty["tracks"] == []


def test_initialize_omits_tracks_for_none_and_all():
    """'all' and None both omit tracks from initialize (no filter at wire level)."""
    assert "tracks" not in _capture_initialize_request()
    assert "tracks" not in _capture_initialize_request(tracks=None)
    assert "tracks" not in _capture_initialize_request(tracks="all")
...
def test_forward_subagent_text_option_reaches_initialize(enabled):
    """HaijunAgentOptions.forward_subagent_text is plumbed through query()."""
    ...
    async def _test():
        ...
        with (
            ...
            async for _ in query(
                prompt="Hello",
                options=HaijunAgentOptions(forward_subagent_text=enabled),
            ):
                pass
    ...
    def test_result_with_inflight_task_keeps_stdin_open(self, drain_frame):
        ...
        async def _test():
            ...
            with (
                ...
                async for msg in query(
                    prompt="Hello",
                    options=HaijunAgentOptions(mcp_servers={"greeter": server}),
                ):
                    messages.append(msg)
            ...
            with (
                ...
                messages = [
                    ...
                    async for msg in query(
                        ...
                        options=HaijunAgentOptions(can_use_tool=callback),
                    )
                ]
            ...
            with (
                ...
                with anyio.fail_after(5):
                    messages = [
                        ...
                        async for msg in query(
                            prompt=prompt_stream(),
                            options=HaijunAgentOptions(can_use_tool=allow_all),
                        )
                    ]
            assert messages == []
            assert ended.is_set()
...
class TestProcessExitAfterErrorResult:
    ...
    def test_process_error_after_error_result_uses_result_error_text(self):
        async def _test():
            ...
            with pytest.raises(
                ProcessError,
                match=r"Haijun Code returned an error result: "
    ...
    def test_api_error_result_uses_result_text_not_success_subtype(self):
        ...
        async def _test():
            ...
            text = str(err)
            assert (
                "Haijun Code returned an error result: "
    ...
    def test_api_error_result_without_text_uses_http_status(self):
        ...
        async def _test():
            ...
            with pytest.raises(
                ProcessError,
                match=r"Haijun Code returned an error result: API error \(HTTP 529\)",
            ):
                async for _ in q.receive_messages():
                    pass
            await q.close()
    ...
    def test_blank_errors_fall_back_to_subtype(self):
        ...
        async def _test():
            ...
            with pytest.raises(
                ProcessError,
                match=r"Haijun Code returned an error result: error_during_execution",
            ):
                async for _ in q.receive_messages():
                    pass
            await q.close()
    ...
    def test_malformed_errors_field_does_not_break_reader(self, errors, expected):
        ...
        async def _test():
            ...
            with pytest.raises(
                ProcessError,
                match=rf"Haijun Code returned an error result: {expected} \(",
            ):
                async for _ in q.receive_messages():
                    pass
            await q.close()
    ...
    def test_session_state_changed_after_error_result_preserves_replacement(self):
        ...
        async def _test():
            ...
            with pytest.raises(
                Exception, match=r"Haijun Code returned an error result"
            ):
                async for _ in q.receive_messages():
                    pass
            await q.close()
