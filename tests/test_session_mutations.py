...
from haijun_agent_sdk._internal.session_mutations import (
    _sanitize_unicode,
    _try_append,
)
from haijun_agent_sdk._internal.sessions import _sanitize_path, get_session_messages
...
class TestRenameSession:
    ...
    def test_invalid_session_id_raises(self, haijun_config_dir: Path):
    ...
    def test_empty_title_raises(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_session_not_found_raises(self, haijun_config_dir: Path, tmp_path: Path):
        """Session not found raises FileNotFoundError."""
        project_path = str(tmp_path / "proj")
        Path(project_path).mkdir(parents=True)
        _make_project_dir(haijun_config_dir, os.path.realpath(project_path))
    ...
    def test_appends_custom_title_entry(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_last_wins_via_list_sessions(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_search_all_projects(self, haijun_config_dir: Path):
    ...
    def test_skips_zero_byte_stub(self, haijun_config_dir: Path):
        ...
        proj_a = _make_project_dir(haijun_config_dir, "/aaa/project")
        proj_z = _make_project_dir(haijun_config_dir, "/zzz/project")
    ...
    def test_compact_json_format(self, haijun_config_dir: Path, tmp_path: Path):
...
class TestTagSession:
    ...
    def test_empty_tag_raises(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_appends_tag_entry(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_tag_trimmed(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_none_clears_tag(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_last_wins(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_unicode_sanitization(self, haijun_config_dir: Path, tmp_path: Path):
...
class TestDeleteSession:
    ...
    def test_session_not_found_raises(self, haijun_config_dir: Path):
        sid = str(uuid.uuid4())
        with pytest.raises(FileNotFoundError, match="not found"):
            delete_session(sid)

    def test_deletes_session_file(self, haijun_config_dir: Path, tmp_path: Path):
        project_path = str(tmp_path / "proj")
        Path(project_path).mkdir(parents=True)
        project_dir = _make_project_dir(
            haijun_config_dir, os.path.realpath(project_path)
    ...
    def test_deletes_without_directory(self, haijun_config_dir: Path):
        """Searches all project directories when no directory is given."""
        project_dir = _make_project_dir(haijun_config_dir, "/any/project")
        sid, file_path = _make_session_file(project_dir)
        assert file_path.exists()
        delete_session(sid)
        assert not file_path.exists()

    def test_no_longer_in_list_sessions(self, haijun_config_dir: Path, tmp_path: Path):
        project_path = str(tmp_path / "proj")
        Path(project_path).mkdir(parents=True)
        project_dir = _make_project_dir(
            haijun_config_dir, os.path.realpath(project_path)
...
class TestForkSession:
    ...
    def test_invalid_up_to_message_id_raises(self, haijun_config_dir: Path):
        with pytest.raises(ValueError, match="Invalid up_to_message_id"):
            fork_session(str(uuid.uuid4()), up_to_message_id="not-valid")

    def test_fork_creates_new_session(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_fork_remaps_uuids(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_fork_up_to_message_id(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_fork_custom_title(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_fork_session_id_in_entries(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_fork_forked_from_field(self, haijun_config_dir: Path, tmp_path: Path):
    ...
    def test_fork_without_directory(self, haijun_config_dir: Path):
        """Searches all project directories when no directory is given."""
        project_dir = _make_project_dir(haijun_config_dir, "/any/project")
        sid, _, _ = _make_transcript_session(project_dir)
    ...
    def test_fork_clears_stale_fields(self, haijun_config_dir: Path, tmp_path: Path):
