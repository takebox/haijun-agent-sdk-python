...
from haijun_agent_sdk._internal import sdk_mcp_bridge
from haijun_agent_sdk._internal._mcp_compat import MCP_MAJOR
...
async def test_max_result_size_chars_annotation_flows_to_cli():
    ...
    ``_meta["haijun/maxResultSizeChars"]``, which it reads to keep a large
    ...
    assert tools["get_large_schema"]["_meta"] == {"haijun/maxResultSizeChars": 500_000}
    assert tools["get_large_schema"]["annotations"]["readOnlyHint"] is True
    assert tools["hand_built"]["_meta"] == {"haijun/maxResultSizeChars": 333}
    assert "haijun/maxResultSizeChars" not in tools["small_tool"].get("_meta", {})
...
async def test_max_result_size_chars_declared_as_a_field_on_a_subclass_flows_too():
    ...
    assert listed["_meta"] == {"haijun/maxResultSizeChars": 444}
...
async def test_tool_annotations_take_either_spelling_on_every_mcp_version():
    ...
    for name in ("camel", "snake"):
        ...
        assert listed[name]["_meta"] == {"haijun/maxResultSizeChars": 11}
...
class TestPythonTypeToJsonSchema:
    ...
    def test_annotated_in_dict_style_schema(self) -> None:
        ...
        from haijun_agent_sdk import create_sdk_mcp_server, tool
